...
function Field({ label, name, type = 'text', defaultValue, placeholder }: {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {type === 'textarea' ? (
        <textarea
          id={name} name={name} defaultValue={defaultValue ?? ''} rows={4}
...
export function AuthorForm({ action, author }: AuthorFormProps) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5 max-w-lg">
      ...
      <Field label="Nom" name="name" defaultValue={author?.name} />
      <Field label="Slug" name="slug" defaultValue={author?.slug} placeholder="victor-hugo" />
      <Field label="Biographie" name="bio" type="textarea" defaultValue={author?.bio} />
      <Field label="URL de l'avatar" name="avatar_url" defaultValue={author?.avatar_url} />
      <div className="flex gap-3">
        <Button type="submit" variant="primary" size="md" loading={pending}>
          {author?.id ? 'Enregistrer' : 'Créer l\'auteur'}
        </Button>
